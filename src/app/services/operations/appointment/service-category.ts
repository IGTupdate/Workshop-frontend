...
export const createServiceCategory = async (name: string, isActive?: boolean): Promise<void> => {
    try {
...
export const updateServiceCategory = async (_id: string, name?: string, isActive?: boolean): Promise<void> => {
    try {
        if(!name && !isActive) {toast.error("NO CHANGES FOUND"); return}
        const response = await apiConnector({
            method: "POST",
