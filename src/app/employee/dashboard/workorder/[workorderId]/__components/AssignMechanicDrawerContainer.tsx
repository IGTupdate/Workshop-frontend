...
import {
  assignMechanicWorkorder,
  getEmployeeWorkingStatus,
...
const AssignMechanicDrawerContainer = (props: Props) => {
  ...
  const [openChildDrawer, setOpenChildDrawer] =
  ...
  useEffect(() => {
    (async function () {
      try {
        const response = (await getEmployeeWorkingStatus(
          employeeRole.mechanic,
        )) as TEmployeeWorkStatus[];
  ...
  const handleAssignMechanicToWorkorder = async () => {
    ...
    try {
      ...
      const response = await assignMechanicWorkorder(workOrderId, {
        mechanics: assign_mechanic_id,
      });
      ...
      const current_assigned_mechanics = selectedMechanics.map((selected) => {
        if (
          ...
          typeof props.assigned_mechanics[0] === "string"
        ) {
      ...
      props.handleUpdateWorkOrderData("mechanicId", [
        ...props.assigned_mechanics,
        ...current_assigned_mechanics,
      ]);
              ...
              <li key={index}>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="flex gap-4">
                    ...
                    <Button
                      disabled={
                        checkIfAlreadyAssigned(mechanic._id) ? true : false
                      }
                      onClick={() => {
        ...
        <div className="mb-4">
          ...
          {selectedMechanics.length > 0 ? (
            <ul className="grid grid-cols-3 gap-4">
              {selectedMechanics.length > 0 &&
                selectedMechanics.map((el) => {
                  return (
                    <li
                      key={el._id}
                      className="flex gap-4 justify-between items-center font-semibold p-2 rounded-md bg-yellow-200"
                    >
                      {el.fullName}
                      <button
                        type="button"
        ...
        {openChildDrawer ? (
          <div>
            ...
            <div className="my-4">
              ...
              {openChildDrawer.assigned_workOrder.length > 0 ? (
                openChildDrawer.assigned_workOrder.map((el, index) => {
                  return (
                    <div
                      key={index}
                      className="flex gap-4 justify-between items-center pb-4"
                    >
                      <div
                        className={`flex w-1/2 flex-col ps-6 relative before:content-[''] before:absolute ${openChildDrawer.assigned_workOrder.length - 1 === index ? "before:h-0" : "before:h-full"} before:w-[1px] before:bg-slate-300 before:left-[0.2em] before:top-[1.15rem]
                                              `}
                      >
                      ...
                      <div className="flex w-1/2 flex-col">
                        ...
                        <p>
                          {el.ramdId && typeof el.ramdId !== "string"
                            ? el.ramdId?.name
                            : "-"}
                        </p>
                      </div>
