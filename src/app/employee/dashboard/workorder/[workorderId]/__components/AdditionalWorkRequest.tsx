                        ...
                        : <div>
                            {
                                workRequests.map((el, index) => {
                                    return <div key={index} className='border rounded-md p-2 mb-3'>
                                        <div className='grid grid-cols-2 gap-3'>
                                            <DescriptionItem title='Estimated Cost' content={el.estimatedCost} />
                                            <div className='relative'>
                                                <Tag className='absolute top-2 right-2'>{el.status}</Tag>
                                            </div>
                                            <div className='col-span-2'>
                                                <DescriptionItem title='Description' content={el.description} />
                                            </div>
                                            <div>
                                                ...
                                                <ul>
                                                    {
                                                        el.tasks.map((task)=>{
                                                            return <li key={task._id} className='flex justify-between'>
                                                            <p>{task.title} {task.critical && "(Critical)"}</p>
                                                            <b>{task.approved}</b>
                                                        </li>
                                                        })
                                                    }
