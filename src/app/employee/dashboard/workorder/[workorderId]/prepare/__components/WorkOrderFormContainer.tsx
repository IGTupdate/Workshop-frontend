...
import { TworkorderPrepare, workorderPrepareYupSchema } from '@/app/validators/workorder';
...
const WorkOrderFormContainer = (props: Props) => {
    ...
    const { control, handleSubmit, formState: { errors }, setValue, getValues, watch } = useForm<TworkorderPrepare>({
        defaultValues: {
    ...
    const appendValueInArrayField = (fieldName: keyof TworkorderPrepare, data: any) => {
    ...
    const removeValueInArrayField = (fieldName: keyof TworkorderPrepare, index: number) => {
        const prvVal = getValues(fieldName);
        if (prvVal && typeof prvVal === "object") {
        ...
        <div>
            <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className='w-full grid md:grid-cols-2 grid-cols-1  gap-4'>
                    ...
                    <div className=' mb-4'>
                        <InputField
                            ...
                            placeholder={"25"}
                            control={control}
                            error={errors.odometerReading ? errors.odometerReading.message : ""}
                        />
                    </div>
                </div>
                ...
                <div>
                    <div className='mb-4 md:w-1/2'>
                        <InputFieldWithButton
                            handleButtonClick={(value: string) => {
                                if (value) {
                                    appendValueInArrayField("partsRequested", { partName: value })
                                }
                            }}
                            name={"partsRequested"}
                        ...
                        <ul className='mt-2'>
                            {
                                watch("partsRequested")?.map((part, index) => {
                                    return <li key={index} className='relative flex items-center gap-4 ps-6 before:content=[""] before:absolute before:left-0 before:top-1/2 before:translate-y-[-50%] before:h-[10px] before:w-[10px] before:rounded-full before:bg-yellow-300'>
                                        <p>{part.partName}</p> <button onClick={() => {
                ...
                <div className='w-full grid md:grid-cols-2 grid-cols-1  gap-4'>
                    ...
                    <div className=' mb-4'>
                        <TimeField
                            ...
                            error={errors.estimatedTimeOfCompletion ? errors.estimatedTimeOfCompletion.message : ""}
                        />
                    </div>
                </div>
                ...
                <div className='flex justify-end mt-4'>
                    <Button disabled={loading} htmlType='submit' type='primary'>Save</Button>
                </div>
            </form>
        </div>
    )
