...
type Props = {
  ...
  setActiveAdvisor: React.Dispatch<
    React.SetStateAction<TEmployeeWorkStatus | null>
  >;
};
        ...
        {props.activeAdvisor ? (
          <div>
            <div className="">
              ...
              <div>
                <Title level={5} className="mb-4 text-base font-bold">
                  Assigned Work Order
                </Title>
                <ul className="">
                  {props.activeAdvisor &&
                  props.activeAdvisor.assigned_workOrder.length > 0 ? (
                    props.activeAdvisor.assigned_workOrder.map((el, index) => {
                      return (
                        ...
                        >
                          <div
                            className={`flex w-1/2 flex-col ps-6 relative before:content-[''] before:absolute ${(props.activeAdvisor && props.activeAdvisor.assigned_workOrder.length - 1) === index ? "before:h-0" : "before:h-full"} before:w-[1px] before:bg-slate-300 before:left-[0.2em] before:top-[1.15rem]
                                                    `}
                          >
