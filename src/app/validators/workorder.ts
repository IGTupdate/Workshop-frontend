...
export const workOrderAdditionalWorkCreateRequest = Yup.object({
    ...
    tasks: Yup.array(Yup.object({
        ...
        partsRequired: Yup.array(Yup.object({
            partId: Yup.string().optional(),
            partName: Yup.string().required("Part Name is required"),
            price: Yup.number().optional()
        })).default([])
    })).default([]),
    estimatedCost: Yup.number().required("Estimated Cost is required for the additional Work").min(0, "Cost should be greater than 0")
})
...
export type TworkOrderAdditionalWorkCreateRequest = Yup.InferType<typeof workOrderAdditionalWorkCreateRequest>