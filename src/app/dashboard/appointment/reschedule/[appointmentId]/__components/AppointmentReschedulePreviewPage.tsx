...
type Props = {
  ...
  setAppointmentRescheduleData: React.Dispatch<
...
const AppointmentRescheduleConfirmation = (props: Props) => {
  ...
  const [
    appointmentRescheduleConfirmationData,
    setAppointmentRescheduleConfirmationData,
  ] = useState<TappointmentRescheduleConfirmationData>({
    vehicle: null,
    customer: null,
    slot_details: null,
  });
  ...
  useEffect(() => {
    ...
    if (
      props.appointmentRescheduleData.calender_id &&
  ...
  const handleRescheduleAppointment = async () => {
    try {
      setLoading(true);
      await rescheduleAppointment(props.appointmentId, {
        calender_id: props.appointmentRescheduleData.calender_id,
        slot_id: props.appointmentRescheduleData.slot_id,
      });
      userRole === "customer"
        ? router.push(`/dashboard/appointment/${props.appointmentId}`)
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <DescriptionItem
            title="Email"
            content={
              appointmentRescheduleConfirmationData.customer?.email || "-"
            }
          />
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <DescriptionItem
            ...
            content={
              appointmentRescheduleConfirmationData.vehicle
          ...
          <DescriptionItem
            title="Make"
            content={
              appointmentRescheduleConfirmationData.vehicle?.vehicle_make || "-"
            }
          />
          <DescriptionItem
            title="Model"
            content={
              appointmentRescheduleConfirmationData.vehicle?.vehicle_model ||
              "-"
            }
          />
          <DescriptionItem
            title="Owner"
            content={
              appointmentRescheduleConfirmationData.vehicle?.owner || "-"
            }
          />
        </div>
      </div>
