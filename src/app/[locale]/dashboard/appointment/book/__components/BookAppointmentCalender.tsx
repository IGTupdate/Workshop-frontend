...
const BookAppointmentContainer: React.FC = () => {
  ...
  const [appointmentBookingData, setAppointmentBookingData] =
    useState<TAppointmentBook>(
      JSON.parse(
        localStorage.getItem("appointmentBookingData") ||
          JSON.stringify({
            slot_id: "",
            calender_id: "",
            customer_id: "",
            vehicle_id: "",
            service_plans: JSON.parse(
              localStorage.getItem("selectedPlans") || JSON.stringify([]),
            ),
            service_description: [],
  ...
  useEffect(() => {
    const { calender_id, slot_id, customer_id, vehicle_id, showServicePlans } =
      appointmentBookingData;
    if (
      calender_id &&
      slot_id &&
  ...
  return (
    <div>
      ...
      {currentStep === 3 && (
        <CustomerAppointmentBookingConfirmation
          appointmentBookingData={appointmentBookingData}
          setAppointmentBookingData={setAppointmentBookingData}
          setCurrentStep={setCurrentStep}
        />
