            ...
            <ul className="flex flex-wrap justify-between items-center">
              {props.tasks.map((el, index) => {
                return (
                  <li
                    key={index}
                    className='relative flex items-center gap-4 ps-6 before:content=[""] before:absolute before:left-0 before:top-1/2 before:translate-y-[-50%] before:h-[10px] before:w-[10px] before:rounded-full before:bg-yellow-300 w-1/2 mb-2'
                  >
                    <p>{el.title}</p>
                  </li>
                );
