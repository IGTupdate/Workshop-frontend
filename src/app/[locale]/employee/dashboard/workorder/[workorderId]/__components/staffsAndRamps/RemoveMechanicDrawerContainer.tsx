...
const RemoveMechanicDrawerContainer = (props: Props) => {
  ...
  const handleSelectMechanic: GetProp<typeof Checkbox.Group, "onChange"> = (
    mechanic,
  ) => {
    ...
    if (alreadySelected) {
      ...
      const current_selected = mechanics.find(
        (el) => el._id === String(mechanic),
      );
      if (current_selected) {
        setSelectedMechanics((prv) => {
          return [...prv, current_selected];
        });
      }
    }
  };
  ...
  const handleRemoveOrderMechanic = async () => {
    ...
    try {
      ...
      const response = await removeMechanicWorkOrder(workOrderId, {
        mechanics: removed_mechanics_id,
      });
      ...
      const updatedMechanics = props.assigned_mechanics.filter((el) => {
        const required_selected_mechanics = selectedMechanics.find(
          (selected) => {
      ...
      {loading ? (
        ...
        <div>
          ...
          <div className="mt-4 flex justify-end">
            <Button onClick={handleRemoveOrderMechanic} type="primary">
              Remove
            </Button>
          </div>
        </div>
