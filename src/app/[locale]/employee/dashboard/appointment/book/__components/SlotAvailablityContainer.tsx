...
const SlotAvailablityContainer = (props: Props) => {
    ...
    (async function () {
      try {
        const availableSlotsData =
          (await getAvailableSlots()) as TAvailbleSlots[];
  ...
  const prepareSegmentOptions = () => {
    ...
    for (
      let currentDate = startDate, key = 0;
      currentDate <= endDate;
      key++, currentDate.setDate(currentDate.getDate() + 1)
    ) {
        ...
        optionArray.push({
          ...
          label: (
            <Tooltip title="Closed">
              {getFormattedDateForSlotAvailabilityFilter(new Date(currentDate))}
            </Tooltip>
          ),
          disabled: true,
        });
